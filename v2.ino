...
if (mode == 1) // work
{
  for (int i = 0; i < COUNT_MEMORY_CHAR && char_data[i].char_value > 0; i++)
  {
    ...
    if (is_search)
    {
        if (char_data[i].char_value != detected_char) print_port(char_data[i].char_value);
        detected_char = char_data[i].char_value;
        break;
    }
  }
  delay(500);
...
{
  if (active_port == 1) Serial.write(cp);    
  if (active_port == 2) Serial1.write(cp);
}
